...
public class AlbumMapper {
    ...
    private static List<Album.Image> mapImages(List<GetInfoResponse.Image> images) {
        return Optional.ofNullable(images)
                .filter(list -> !list.isEmpty())
                .map(list -> list.stream()
                        .map(img -> new Album.Image(img.size, img.url))
                        .toList())
                .orElse(List.of());
    }
    ...
    private static List<Album.Tag> mapTags(Object tags) {
        ...
        if (tags instanceof Map) {
            ...
            if (tagArray instanceof List) {
                ...
                return tagList.stream()
                        .map(tagMap -> new Album.Tag(
                                (String) tagMap.get("name"),
                                (String) tagMap.get("url")
                        ))
                        .toList();
            }
        }
        ...
        if (tags instanceof GetInfoResponse.Tags tagsObj) {
            return Optional.ofNullable(tagsObj.tag)
                    .filter(list -> !list.isEmpty())
                    .map(list -> list.stream()
                            .map(t -> new Album.Tag(t.name, t.url))
                            .toList())
                    .orElse(List.of());
        }
    ...
    private static Album.Track mapTrack(GetInfoResponse.Track track) {
        ...
        Integer rank = track.attr != null && track.attr.rank != null
                ? parseNumber(track.attr.rank) : null;
        ...
        String streamable = track.streamable != null && track.streamable.text != null
