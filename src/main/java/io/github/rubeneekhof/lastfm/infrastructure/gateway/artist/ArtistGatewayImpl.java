...
/**
 * Gateway implementation for Last.fm Artist API. Orchestrates HTTP → DTO → Domain mapping. Contains
 * zero mapping logic - all mapping is delegated to ArtistMapper.
 */
  ...
  public Artist getInfo(
    ...
    try {
      ...
      GetInfoResponse response =
          getAndParse(http, mapper, "artist.getinfo", params, GetInfoResponse.class);
  ...
  public List<Artist> getSimilar(String artist, String mbid, Boolean autocorrect, Integer limit) {
    try {
      ...
      GetSimilarResponse response =
          getAndParse(http, mapper, "artist.getsimilar", params, GetSimilarResponse.class);
      return ArtistMapper.from(response);
    } catch (LastFmException e) {
      throw new LastFmFailureException(LastFmErrorMapper.map(e.code(), e.getMessage()));
    }
  ...
  public Optional<Artist> getCorrection(String artist) {
    try {
      ...
      GetCorrectionResponse response =
          getAndParse(http, mapper, "artist.getcorrection", params, GetCorrectionResponse.class);
      return ArtistMapper.from(response);
    } catch (LastFmException e) {
      throw new LastFmFailureException(LastFmErrorMapper.map(e.code(), e.getMessage()));
    }
